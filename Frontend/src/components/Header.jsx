...
const Header = () => {
    ...
    <>
      <header className="fixed top-0 left-0 w-full bg-white border-b border-[#E5E7EB] z-50 dark:bg-[#0F172A] dark:text-slate-50 dark:border-slate-700">
        ...
        <div className="h-16 max-w-[90%] mx-auto flex items-center justify-between gap-4">
          ...
          {/* Search (Desktop) */}
          <div className="hidden md:flex flex-1 max-w-xl items-center border border-[#E5E7EB] rounded-md px-3 py-2 focus-within:ring-2 focus-within:ring-[#4F46E5] relative dark:text-slate-50 dark:border-slate-700">
            <input
              value={search}
          ...
          {search.trim() && (
            <div className="absolute top-full left-0 mt-2 w-full bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden dark:bg-slate-900 dark:border-slate-600">
                  ...
                  <div className="max-h-96 overflow-y-auto">
                    {filteredProducts.slice(0, 6).map((product) => (
                      <Link
                        ...
                        className="flex items-center gap-4 px-4 py-3 hover:bg-indigo-50 transition-all duration-200 border-b last:border-none dark:hover:bg-slate-800 dark:border-slate-700"
                      >
                        {/* Product Image */}
                        <div className="w-16 h-16 rounded-xl bg-gray-100 p-2 flex items-center justify-center dark:bg-slate-800">
                          <img
                        ...
                        <div className="flex-1 overflow-hidden">
                          <h3 className="font-semibold text-gray-800 truncate dark:text-slate-50">
                            {product.title}
                          </h3>

                          <p className="text-sm text-gray-500 capitalize dark:text-slate-300">
                            {product.category}
                          </p>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <Link
              ...
              className="relative flex items-center gap-1 p-2 rounded-md hover:bg-gray-100 transition cursor-pointer dark:bg-slate-800 dark:hover:bg-slate-700"
            >
  ...
  "
        ...
        <div className="md:hidden px-[5%] pb-3">
          <div className="flex items-center border border-[#E5E7EB] rounded-md px-3 py-2 dark:border-slate-700 focus-within:ring-2 focus-within:ring-indigo-400">
            <input
              value={search}
              onChange={(e) => dispatch(setSearch(e.target.value))}
              type="text"
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="fixed top-[104px] left-0 w-full bg-white dark:bg-slate-900 shadow-lg md:hidden z-40 mt-3">
          {navLinks.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              onClick={() => setMenuOpen(false)}
              className="block px-6 py-4 border-b hover:bg-gray-50 transition dark:hover:bg-slate-800 dark:border-slate-700"
            >
              {item.name}
            </Link>
          ))}
